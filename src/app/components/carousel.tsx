...
export default function Carousel() {
  return (
    <div className="w-full overflow-hidden cursor-grab active:cursor-grabbing pt-[24px] pb-[48px]  ">
      <motion.div
        className="flex gap-3"
        drag="x"
        dragConstraints={{ left: -140 * cards.length, right: 0 }}
      >
        {[...cards, ...cards].map((card, index) => (
          <div key={index} className="overflow-hidden min-w-[370px] bg-white  shadow-[0px_4px_10px_rgba(0,0,0,0.15)]  rounded-lg">
            <img src={card.img} alt={card.title} className="w-full h-[230px] object-fit" />
            <div className="px-[35px] py-8 bg-white flex flex-col gap-[28px]">
              ...
              <Button variant={index === 0 ? "solid" : "bordered"}
